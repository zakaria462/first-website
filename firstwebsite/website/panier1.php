...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($products as $product) {
        ...
        $sqlQuantity = "SELECT quantité FROM camera WHERE id = '$productId'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($quantity <= $availableQuantity) {
                ...
                $sqlUpdateQuantity = "UPDATE camera SET quantité = '$availableQuantity' WHERE id = '$productId'";
...
<script>
  ...
  function loadCartData() {
    const storedCartData = JSON.parse(getCookie('cartData1'));
    if (storedCartData) {
