...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_SESSION['total_earnings'])) {
    ...
    foreach ($products as $product) {
        ...
        $sqlQuantity = "SELECT quantité FROM accessoires WHERE id = '$productId'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($quantity <= $availableQuantity) {
                ...
                $sqlUpdateQuantity = "UPDATE accessoires SET quantité = '$availableQuantity' WHERE id = '$productId'";
    ...
    $_SESSION['total_earnings'] += $totalCommande;
...
<script>
  ...
  function loadCartData() {
    const storedCartData = JSON.parse(getCookie('cartData3'));
    if (storedCartData) {
  ...
  function populateCartSummary() {
    ...
    totalPriceElement.textContent = `Total: ${totalCommande} Dh`;
