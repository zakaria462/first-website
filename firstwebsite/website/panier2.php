...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($products as $product) {
        ...
        $sqlQuantity = "SELECT quantité FROM iphone WHERE id = '$productId'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if ($quantity <= $availableQuantity) {
                ...
                $sqlUpdateQuantity = "UPDATE iphone SET quantité = '$availableQuantity' WHERE id = '$productId'";
...
<script>
  ...
  function loadCartData() {
    const storedCartData = JSON.parse(getCookie('cartData2'));
    if (storedCartData) {
